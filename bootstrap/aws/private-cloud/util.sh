...
ansible_ssh_config() {
  pushd "${KUBEFORM_ROOT}/terraform/${KUBEFORM_PROVIDER}"
    export KUBEFORM_bastion_ip=$( terraform output bastion.ip )

    # Virtual private cloud CIDR IP.
    ip=$( terraform output -module=vpc vpc_cidr_block )
    export KUBEFORM_network_identifier=$( get_network_identifier "${ip}" )

    cat <<EOF > ssh.config
  Host bastion $KUBEFORM_bastion_ip
    StrictHostKeyChecking  no
    User                   core
    HostName               $KUBEFORM_bastion_ip
    ProxyCommand           none
  ...
  Host $KUBEFORM_network_identifier.*
    StrictHostKeyChecking  no
    ServerAliveInterval    120
    TCPKeepAlive           yes
    ProxyCommand           ssh -q -A -F $(pwd)/ssh.config core@$KUBEFORM_bastion_ip ncat %h %p
    ControlMaster          auto
...
ovpn_client_config() {
  pushd "${KUBEFORM_ROOT}/terraform/${KUBEFORM_PROVIDER}"
    ...
    /usr/bin/env sed -i -e "s/\([0-9]\{1,3\}\.\)\{3\}[0-9]\{1,3\}/${bastion_ip}/g" "${TF_VAR_user-kubeform.ovpn}"

    /usr/bin/open "${TF_VAR_user-kubeform.ovpn}"
